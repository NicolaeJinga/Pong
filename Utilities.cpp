...
namespace Utilities
{
	...
	void RotateSquareMatrixClockwise(int* matrix, int n)
	{
		...
		for(int i = 0 ; i < n ; ++ i)
			for(int j = 0 ; j < n ; ++ j)
				matrix[i*n+j] = a[(n-j-1)*n+i]; // <=> matrix[i][j] = a[n-j-1][i];
		delete[] a;
	}
	...
	void RotateSquareMatrixCounterClockwise(int* matrix, int n)
	{
		...
		for(int i = 0 ; i < n ; ++ i)
			for(int j = 0 ; j < n ; ++ j)
				matrix[i*n+j] = a[j*n+(n-i-1)]; // <=> matrix[i][j] = a[j][n-i-1];
		delete[] a;
	}

	bool Utilities::IsInBoundsAABB(sf::Vector2f point, sf::Vector2f A, sf::Vector2f C)
	{
